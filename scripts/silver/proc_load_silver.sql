	...
	BEGIN TRY
		...
		SELECT 
			...
			CASE 
				...
				ELSE 'n/a' -- normalize the gender values to readable format
			END AS cst_gndr,
			cst_create_date
		FROM T --CTE
		...
		END AS cid,
		CASE 
			WHEN bdate > GETDATE() THEN NULL -- used NULL for bdays greater than today's date
			ELSE bdate
		END AS bdate,
		CASE 
			WHEN UPPER(TRIM(gen)) IN ('F','FEMALE') THEN 'Female' --data normalization/data standarization
		...
		CASE 
			...
			WHEN TRIM(cntry) ='' OR cntry IS NULL THEN 'n/a' --missing values 
			ELSE TRIM(cntry)
